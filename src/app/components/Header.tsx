...
function LanguageSwitcher({ variant = "dark" }: { variant?: "dark" | "light" }) {
  ...
  return (
    <button
      onClick={toggleLang}
      className={`flex items-center gap-1 md:gap-[0.2083vw] px-3 py-1.5 md:px-[0.625vw] md:py-[0.4167vw] rounded-full border-2 font-bold text-xs md:text-[0.7292vw] leading-none md:leading-[1.0417vw] transition-colors whitespace-nowrap ${
        isLight
...
export function Header() {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between h-16 px-4 md:h-[3.3333vw] md:max-w-[80vw] md:mx-auto md:px-[0.8333vw]">
        ...
        <Link to="/" onClick={scrollToTop} className="flex items-center shrink-0">
          <img src={currentLogo} alt="Healing Voice" className="h-8 md:h-[2.1875vw] w-auto object-contain" />
        </Link>
        ...
        <div className="flex md:hidden items-center gap-1.5">
          {menuOpen ? (
            <>
              ...
              >
                <X className="w-6 h-6 text-[#062259]" strokeWidth={2} />
              </button>
            </>
          ) : (
            <>
    ...
    {menuOpen && (
      <div className="md:hidden fixed inset-x-0 top-16 bottom-0 z-40 flex flex-col">
        <div className="bg-white flex flex-col gap-8 px-5 pt-4 pb-8">
          <nav className="flex flex-col gap-8">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => scrollToSection(item.id)}
                className="text-left text-[#062259] font-semibold text-xl leading-[1.2]"
              >
                {item.label}
              </button>
            ))}
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 px-8 py-4 rounded-full bg-[#6276FB] hover:bg-[#4f5fe0] shadow-[0px_1px_2px_rgba(0,0,0,0.05)] text-white font-bold text-lg transition-colors"
          >
            {t("header.cta")}
